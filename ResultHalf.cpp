...
const int S_P = 10;  //ランクSのスコア 値の修正する
const int SCREEN_SIZE_W = 1920;
const int SCREEN_SIZE_H = 1080;//縦横の画面サイズ
const int VOLUME_PAL_SUP = 90;//音量の調整
// 最大透過量
...
void ResultHalf::Draw()
{
	...
	{
		DrawGraph(0, 0, m_evaluationGraphHandle[m_evaluation], TRUE);				//	ランクを表示
	}
	// フェードアウト処理
	if (m_fadeOutFlag)
	{
...
void ResultHalf::Load()
{
	...
	m_logoGraphHandle = LoadGraph("data/img/result_02_png/swimResult/Result _01_backGround.png");				//	背景の画像をセット
	m_guidanceGraphHandle = LoadGraph("data/img/result_02_png/swimResult/Result _Exitword.png");				//	エンタートゥスタートの画像をセット	
	m_bgmSoundHandle = LoadSoundMem("data/sound/Result/SwimResult.mp3");										//	リザルトのBGMをセット
}